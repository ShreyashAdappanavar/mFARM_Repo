...
parser.add_argument('--model', type=str, required=True, help='Base model name in format repo_name/model_name')
parser.add_argument('--curr_task_dir', type=str, required=True, help='Current task dir')
parser.add_argument('--adapter_path', type=str, required=True, help='Path to the directory containing saved PEFT adapter weights')
...
P_FOR_YES_NO = 80  # Number of hadm_id to sample for just Yes/No prediciton
...
print(f"THE ADAPTER PATH THAT WAS PASSED IS: {ADAPTER_PATH}")
...
ALL_RESULTS_DIRECTORY="/home/gokul/Hier-Legal-Graph/mimic_dataset/mimiciv_dataset/2.2/Med_LLM_Fairness_3/All_Results"
...
CURRENT_TASK_DESCRIPTION = "OPIOID ANALGESIC PRESCRIPTION, DEMO INFO AT START"
...
final_data_path = os.path.join(CURRENT_TASK_DIRECTORY, "Dataset_and_code_for_datasets", "ALL_DATASETS", "final_test_dataset.csv")
...
CURRENT_LLM_DIRECTORY = MODEL_NAME.split("/")[1] + "---" +ADAPTER_PATH.split("/")[-1]
...
def setup_model_and_tokenizer(model_name: str, 
    ...
    Args:
        model_name: Base model identifier (e.g., 'BioMistral/BioMistral-7B').
        adapter_path: Path to the saved PEFT adapter directory.
        hf_auth_token: Hugging Face token.
        llm_quantize, quantize_8_bit, quantize_4_bit: Control quantization.
        trust_remote_code: Control remote code execution trust.
    ...
    model, tokenizer = setup_model_and_tokenizer(
        ...
        adapter_path=ADAPTER_PATH, # Path to adapters
    ...
    if llm_quantize:
        ...
        if quantize_8_bit:
            ...
            # If 4-bit quantization is desired.
        ...
        base_model = AutoModelForCausalLM.from_pretrained(
            ...
            token=hf_auth_token,
            ####### BE VEEEEEEERY CAREFUL WITH THIS #########
            trust_remote_code=trust_remote_code
        )
    else:
    ...
    # --- Apply PEFT Adapters ---
    print(f"Loading and applying PEFT adapters from: {adapter_path}")
    # Check if adapter path exists BEFORE trying to load
    if not os.path.isdir(adapter_path):
         raise FileNotFoundError(f"Adapter directory not found at: {adapter_path}")

    # Load the adapters and apply them to the base model
...
def sample_hadm_ids(final_df: pl.DataFrame, p: int, perc_yes: float) -> pl.DataFrame:
    """
    Randomly selects hadm_id values from the dataframe with a specified percentage of "YES" values for GT_FLAG.
    
    This function samples hadm_ids from the provided dataframe such that the resulting dataframe
    ...
    Parameters:
        ...
        p (int): The total number of hadm_id values to sample.
        perc_yes (float): The desired percentage (0-1) of hadm_ids with GT_FLAG="YES".
    
    Returns:
        pl.DataFrame: A new dataframe containing all rows for the selected hadm_id values.
    ...
    # Extract unique hadm_id values based on GT_FLAG conditions
    hadm_id_yes = sorted(final_df.filter(pl.col("GT_FLAG") == "YES").select("hadm_id").unique().to_series().to_list())
    hadm_id_no = sorted(final_df.filter(pl.col("GT_FLAG") == "NO").select("hadm_id").unique().to_series().to_list())


    # Randomly sample the required number of hadm_id values
    sampled_hadm_id_yes = np.random.choice(hadm_id_yes, num_yes, replace=False).tolist() if num_yes > 0 else []
    sampled_hadm_id_no = np.random.choice(hadm_id_no, num_no, replace=False).tolist() if num_no > 0 else []
    
    # Combine selected hadm_id values
    selected_hadm_ids = sampled_hadm_id_yes + sampled_hadm_id_no

    # Filter final_df to include only rows with the selected hadm_id values
    final_final_df = final_df.filter(pl.col("hadm_id").is_in(selected_hadm_ids))
...
def make_exp_dataframes(final_df: pl.DataFrame,
    ...
    """
    Creates a dataframe with a specified number of hadm_ids and YES/NO percentage.
    
    Parameters:
        final_df (pl.DataFrame): The complete dataframe containing all prompts.
        p_for_yes_no (int): The total number of hadm_id values to sample for the 
    ...
    df_yes_no = sample_hadm_ids(final_df=final_df, p=p_for_yes_no, perc_yes=perc_yes)
...
def process_row(row, tokenizer: PreTrainedTokenizer, model: PreTrainedModel, 
    ...
    Parameters:
        row (dict or pd.Series): Row containing 'prompt', 'hadm_id', 'gender', 'race', and 'GT_FLAG'.
    ...
    prompt_result["hadm_id"] = row["hadm_id"]
...
def get_results_for_a_dataframe(df: pl.DataFrame, tokenizer: PreTrainedTokenizer, 
    ...
    # Process all prompts grouped by hadm_id
    groups = df.group_by("hadm_id")
    num_total_hadm = df["hadm_id"].n_unique()
...
# ------------------------------------------------
# 1) HELPER FUNCTION: Identify base row(s) per hadm_id
# ------------------------------------------------
def identify_base_rows(group: pd.DataFrame) -> pd.Series:
    ...
    We expect exactly 1 such row per hadm_id.
    """
    return group['race'] == "BASE"


# ------------------------------------------------
# 2) HELPER FUNCTION: Compute L2 Norm & IBD for a single hadm_id
# ------------------------------------------------
def compute_l2_norm_and_ibd_for_one_hadm(df_for_hadm: pd.DataFrame, prob_col: str) -> Tuple[float, float]:
    """
    Given a subset of rows (one hadm_id) and a probability column (e.g. 'yes_initial_prob'),
    compute:
    ...
    if base_mask.sum() != 1:
        raise ValueError(
            f"[HADM={df_for_hadm['hadm_id'].iloc[0]}] Expected exactly 1 base row, "
...
def compute_all_metrics(df: pd.DataFrame, demographic_dict: dict, prob_cols: Tuple[str, ...] = ('yes_initial_prob', 'yes_final_prob')) -> pd.DataFrame:
    ...
    Main function to compute:
      1) L2 Norm (average across hadm_ids)
      2) Intersection Bias Discrepancy (IBD) (average across hadm_ids)
    ...
    demographic categories. We'll compute M*N + 1 to check the row count per hadm_id.
    ...
    # 2) Data integrity check: each hadm_id must have exactly M*N + 1 rows
    counts_per_hadm = df.groupby('hadm_id').size()
    invalid_hadm_ids = counts_per_hadm[counts_per_hadm != expected_count_per_hadm].index.tolist()
    if invalid_hadm_ids:
        raise ValueError(
            f"The following hadm_id(s) do not have exactly {expected_count_per_hadm} rows: {invalid_hadm_ids}. "
    ...
    for prob_col in prob_cols:
        # A) Compute L2 Norm & IBD (averaged across hadm_ids)
        l2_values = []
        ibd_values = []
...
def run(
    ...
    MODEL_NAME: str, #
    ADAPTER_PATH: str, # Added adapter path
    ...
    P_FOR_YES_NO : int
        Total number of hadm_ids to sample for Yes/No predictions.
    P_FOR_EXPLANATIONS : int
        Total number of hadm_ids to sample for explanations.
    YES_PERCENTAGE : float
    ...
    model, tokenizer = setup_model_and_tokenizer(
        ...
        adapter_path=ADAPTER_PATH,  # Pass adapter path
        hf_auth_token=HF_TOKEN,
        llm_quantize=LLM_QUANTIZE,
        quantize_8_bit=QUANTIZE_8_BIT,
        quantize_4_bit=QUANTIZE_4_BIT,
